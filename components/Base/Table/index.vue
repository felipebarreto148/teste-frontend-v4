  ...
  <section class="h-fit w-full overflow-y-auto">
    <table class="w-full border-collapse">
      ...
      <TransitionGroup
        ...
        appear
      >
        <tr v-for="(row, rowIndex) in data" :key="rowIndex" class="rounded-lg hover:bg-grey-50 transition-all duration-150">
          <td
            v-for="(column, colIndex) in headers"
            :key="colIndex"
            class="whitespace-nowrap p-6 text-[15px] font-normal text-grey-700"
          >
            <slot :name="column.name" :row="row" :value="row[column.name]">
              {{ row[column.name] }}
            </slot>
          </td>
        </tr>
